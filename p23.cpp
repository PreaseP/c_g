...
GLvoid drawScene()
{
	...
	// --- 뷰포트 1: 원근 투영 (왼쪽) ---
	glViewport(0, 0, WinX * 2 / 3, WinY);
	glm::mat4 vT_persp = glm::rotate(glm::mat4(1.0f), glm::radians(rotC), glm::vec3(0.0f, 1.0f, 0.0f)) *
		glm::lookAt(cameraPos, cameraDirection, cameraUp) *
		glm::rotate(glm::mat4(1.0f), glm::radians(rotCR), glm::vec3(0.0f, 1.0f, 0.0f));
	glUniformMatrix4fv(viewLoc, 1, GL_FALSE, glm::value_ptr(vT_persp));

	glm::mat4 pT_persp = glm::perspective(glm::radians(45.0f), (float)(WinX * 2 / 3) / (float)WinY, 0.1f, 100.0f);
	glUniformMatrix4fv(projLoc, 1, GL_FALSE, glm::value_ptr(pT_persp));
	drawTank();

	// --- 뷰포트 2: 평면도 (오른쪽 위) ---
	glViewport(WinX * 2 / 3, WinY / 2, WinX / 3, WinY / 2);
	glm::mat4 vT_top = glm::rotate(glm::mat4(1.0f), glm::radians(rotC), glm::vec3(0.0f, 1.0f, 0.0f)) *
		glm::lookAt(glm::vec3(0.0f, 20.0f, -5.0f), glm::vec3(0.0f, 0.0f, -5.0f), glm::vec3(0.0f, 0.0f, -1.0f)) *
		glm::rotate(glm::mat4(1.0f), glm::radians(rotCR), glm::vec3(0.0f, 1.0f, 0.0f));
	glUniformMatrix4fv(viewLoc, 1, GL_FALSE, glm::value_ptr(vT_top));

	glm::mat4 pT_ortho = glm::ortho(-12.0f, 12.0f, -12.0f, 12.0f, 0.1f, 100.0f);
	glUniformMatrix4fv(projLoc, 1, GL_FALSE, glm::value_ptr(pT_ortho));
	drawTank();

	// --- 뷰포트 3: 정면도 (오른쪽 아래) ---
	glViewport(WinX * 2 / 3, 0, WinX / 3, WinY / 2);
	glm::mat4 vT_front = glm::rotate(glm::mat4(1.0f), glm::radians(rotC), glm::vec3(0.0f, 1.0f, 0.0f)) *
		glm::lookAt(glm::vec3(0.0f, 0.0f, 20.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f)) *
		glm::rotate(glm::mat4(1.0f), glm::radians(rotCR), glm::vec3(0.0f, 1.0f, 0.0f));
	glUniformMatrix4fv(viewLoc, 1, GL_FALSE, glm::value_ptr(vT_front));
	// 평면도와 동일한 직교 투영 행렬 사용
	glUniformMatrix4fv(projLoc, 1, GL_FALSE, glm::value_ptr(pT_ortho));
	drawTank();
